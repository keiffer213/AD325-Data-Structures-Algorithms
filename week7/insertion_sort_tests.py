import unittest
from insertion_sort_keiffer import insertion_sort, generate_rand_array
...
class TestSelectionSort(unittest.TestCase):
  ...
  def test_sort_rand_arr(self):
    ...
    self.assertEqual(insertion_sort(self.arr.copy()), sorted(self.arr))

  def test_sort_sorted_incr(self):
    """Test sorting an already sorted array"""
    self.assertEqual(insertion_sort(self.arr2.copy()), self.arr2)

  def test_sort_sorted_decr(self):
    """Test Sorting an array in descending order"""
    self.assertEqual(insertion_sort(self.arr3.copy()), sorted(self.arr3))

  def test_sort_same_elements(self):
    """Test sorting an array where all elements are the same"""
    self.assertEqual(insertion_sort(self.arr4.copy()), self.arr4)
    
  def test_sort_edge_cases(self):
    """Test sorting an empty array and an array with a single element"""
    self.assertEqual(insertion_sort(self.arr5.copy()), self.arr5)
    self.assertEqual(insertion_sort(self.arr6.copy()), self.arr6)
  ...
  def test_stability(self):
    ...
    self.assertIsNot(insertion_sort(arr.copy()), expected)
    # print(arr, insertion_sort(arr.copy()), expected)
